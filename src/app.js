...
const getHashratesLast7D = async () => {
  ...
  try {
    ...
    const hs = data.slice(data.length - 168).reduce(
      (acc, currHs, i) => {
        ...
        const hs24H = i < 24 ? acc.hs24H + hashrate : acc.hs24H;
        const hs7D = i < 168 ? acc.hs7D + hashrate : acc.hs7D;
        return { hs1H, hs6H, hs24H, hs7D };
      },
      { hs1H: 0, hs6H: 0, hs24H: 0, hs7D: 0 }
    );
    return {
      hs1H: getHashrateString(hs.hs1H),
      hs6H: getHashrateString(hs.hs6H / 6),
      hs24H: getHashrateString(hs.hs24H / 24),
...
const getFinalString = async (now = false) => {
  ...
  const { hs1H, hs6H, hs24H, hs7D } = await getHashratesLast7D();
  const { maxHs, minHs } = await getMinMax();
  if (now) return `Now: ${hsNow}`;
  return (
    `Now: ${hsNow}\n` +
    `1h: ${hs1H}\n` +
    `6h: ${hs6H}\n` +
